    ...
    try:
        with open(caminho_pgn_entrada, "r", encoding="utf-8") as arquivo_pgn:
            ...
            while True:
                ...
                nome_arquivo_abertura = f"{nome_pasta_seguro}.pgn"
                caminho_arquivo_abertura = os.path.join(pasta_abertura, nome_arquivo_abertura)
                with open(caminho_arquivo_abertura, "a", encoding="utf-8") as arquivo_saida:
                    exportador = chess.pgn.FileExporter(arquivo_saida)
                    jogo.accept(exportador)
                if contador_jogos % 1000 == 0:
                    print(f"⭐ [Estrela do Mar] Progresso: {contador_jogos} partidas categorizadas em suas pastas...")
        print(f"⭐ [Estrela do Mar] Concluído! {contador_jogos} jogos organizados em arquivos unificados.")
